...
static int configured_tile_pixels(int pixel_height, int pixel_width) {
    ...
    int max_extent = pixel_height > pixel_width ? pixel_height : pixel_width;
    if (value && *value) {
        char *end = NULL;
        long pixels = strtol(value, &end, 10);
        /* Multi-tile plans above 320 px quilt on large canvases. A single
         * full-frame tile cannot seam — allow up to the frame extent when set
         * explicitly (e.g. H3_VAE_TILE_PIXELS=448 on Live). Measured slower
         * than 2×2@256 on M3 Ultra, so this stays opt-in. */
        long cap = max_extent > 320 ? max_extent : 320;
        if (end && !*end && pixels >= TILE_PIXELS && pixels <= cap &&
