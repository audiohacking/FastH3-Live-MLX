...
h3_gpu *h3_gpu_create(const char *shader_source_path,
    ...
    @autoreleasepool {
        ...
        if (source) {
            ...
            /* TensorOps was gated on the substring "M5". M3 Ultra / M4 report
             * Metal 4 and Apple GPU family >= 9; try enabling there too so
             * int8 DiT kernels are available for FastH3 Live. H3_NAX=0 disables.
             * H3_FORCE_TENSOROPS=1 forces compile even on older names. */
            BOOL m5 = [gpu.device.name rangeOfString:@"M5"].location !=
                      NSNotFound;
            BOOL m4 = [gpu.device.name rangeOfString:@"M4"].location !=
                      NSNotFound;
            BOOL m3_ultra =
                [gpu.device.name rangeOfString:@"M3 Ultra"].location !=
                NSNotFound;
            BOOL metal4_family = NO;
            if (@available(macOS 15.0, *)) {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wunguarded-availability-new"
                /* MTLGPUFamilyMetal4 == 5004 on recent SDKs; fall back to
                 * Apple8/9 probes when the enum is unavailable. */
                if ([gpu.device supportsFamily:(MTLGPUFamily)5004] ||
                    [gpu.device supportsFamily:MTLGPUFamilyApple9] ||
                    [gpu.device supportsFamily:MTLGPUFamilyApple8])
                    metal4_family = YES;
#pragma clang diagnostic pop
            }
            BOOL force = getenv("H3_FORCE_TENSOROPS") != NULL;
            BOOL wantsTensorOps =
                (m5 || m4 || m3_ultra || metal4_family || force) &&
...
int h3_gpu_is_m5(const h3_gpu *opaque) {
    /* Name is historical: callers use this as "TensorOps / int8 class GPU".
     * FastH3 Live on M3 Ultra enables TensorOps via Metal 4 family probes;
     * treat that the same as M5 for feature gates. */
    if (!opaque) return 0;
    H3GPU *gpu = GPU((h3_gpu *)(void *)opaque);
    if (gpu.tensorOpsEnabled) return 1;
    return [gpu.device.name rangeOfString:@"M5"].location != NSNotFound;
}

int h3_gpu_has_tensor_ops(const h3_gpu *opaque) {
    if (!opaque) return 0;
    H3GPU *gpu = GPU((h3_gpu *)(void *)opaque);
    return gpu.tensorOpsEnabled ? 1 : 0;
}
